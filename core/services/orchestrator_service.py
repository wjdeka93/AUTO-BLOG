...
`apps/orchestrator/main.py` stays intentionally thin and forwards HTTP requests here.
This module turns request payloads into concrete work by calling the lower-level
pipeline, generation, source-fetching, and style-extraction services.
"""
...
# Pipeline runs are persisted so the caller can check status later via `/pipelines/{run_id}`.
run_store = RunStore(PROJECT_ROOT / "data" / "runs")


def health_payload() -> dict[str, str]:
    """Return a tiny health payload used by the health-check endpoint."""

    return {"service": "orchestrator", "status": "ok"}


def fetch_sources_job(payload: SourceFetchRequest) -> dict[str, object]:
    """Fetch every source URL listed in the given file and save the raw text locally."""
...
def extract_post_style_job(payload: PostStyleFromUrlRequest) -> dict[str, object]:
    """Extract one post_style JSON from a single blog URL.

    The user can omit output paths; in that case we derive them from the normalized
    Naver `post_id` so the rest of the pipeline can discover the files predictably.
    """
...
def rebuild_main_style_job(payload: MainStyleRebuildRequest) -> dict[str, object]:
    """Rebuild the author-level style summary from all saved post_style files."""
...
def rebuild_sub_style_job(payload: SubStyleRebuildRequest) -> dict[str, object]:
    """Rebuild grouped sub-styles such as category-specific tone variations."""
...
def generate_job(payload: GenerateRequest) -> dict[str, Any]:
    """Generate one markdown post using style assets and optional retrieval context."""
...
def run_pipeline_job(payload: PipelineRunRequest) -> RunRecord:
    """Run the end-to-end pipeline and persist a run record.

    A pipeline run may include source fetching, post_style extraction, main/sub style
    rebuilding, and a final generation step. Persisting the run record makes the flow
    easier to inspect from the outside and keeps failures observable.
    """
...
def get_pipeline_run_job(run_id: str) -> RunRecord:
    """Load a previously saved run record or raise a 404 for the API layer."""
